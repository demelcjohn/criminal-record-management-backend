...
const court_login = async (req, res) => {
  const data = req.body;
  try {
    citizen_user = await court.findOne({ UID: data.UID });
    if (citizen_user === null) {
      return res.status(201).json({ msg: "User Not Found" });
    }
  } catch (e) {
...
const police_login = async (req, res) => {
  const data = req.body;
  try {
    citizen_user = await police.findOne({ UID: data.UID });
    if (citizen_user === null) {
      return res.status(201).json({ msg: "User Not Found" });
    }
  } catch (e) {
    return res.status(401).json({ msg: "police not found", err: e.message });
  }
